...
class QueueManager:
    ...
    @staticmethod
    def get_waiting_by_doctor(db: Session, doctor_id: str) -> List[Consultation]:
        """
        NEW: Alias method to match the call in main.py.
        Returns the list of waiting patients for a specific doctor.
        """
        return QueueManager.get_waiting_patients(db, doctor_id=doctor_id)

    @staticmethod
    def get_current_patient(db: Session, doctor_id: str) -> Optional[Consultation]:
        """
        NEW: Resolves the AttributeError in main.py.
        Fetches the patient currently 'in_progress' with this doctor.
        """
